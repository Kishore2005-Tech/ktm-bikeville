...
export default function useTilt(ref, hoverSelector, options = {}) {
  useEffect(() => {
    ...
    const cleanups = hosts.map((host) => {
      ...
      // Scale in immediately on entry too - onMove alone only fires once the
      // pointer actually moves inside the card, so a hover that lands and
      // holds still (a real risk with a synthetic/test move, or just a
      // careful mouse) would otherwise show no zoom at all.
      const onEnter = () => quickScale(liftScale)
      const onLeave = () => {
        quickX(0)
        quickY(0)
        quickScale(1)
